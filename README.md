...
- **地图与路线规划**：百度地图 GL 版本，城市级定位展示，支持活动间路线规划
...
- **AI 生成**：调用阿里云百炼（Qwen）生成结构化行程 JSON：
  ...
  - 按天拆分的活动列表（时间、标题、描述、地点、花费、经纬度坐标）
  - 住宿/交通/餐饮/门票/其他的总览估算
- **旅行计划页面展示**：
  - 展示每天的日程建议和费用估算
  - 百度地图展示城市级定位
  - 每条活动包含经纬度信息，支持选择任意两条活动作为起点和终点进行路线规划
- **路线规划功能**：
  - 在每条活动旁边提供"设为起点"和"设为终点"按钮（仅当活动有经纬度时显示）
  - 选择起点和终点后，点击"规划路线"按钮在地图上查看详细路线
  - 支持显示驾车路线、起点/终点标记、路线动画（动画小车沿路线移动）
  - 主要支持中国境内的路线规划；对于国外路线，会显示虚线连接作为位置参考
...
## 🧱 技术栈
- 前端：Vue 3、Vue Router、Pinia、ECharts、百度地图 GL
...
- 地图不显示：检查百度地图 API Key 是否正确加载，查看浏览器控制台错误信息。
- 路线规划失败（状态码5）：百度地图路线规划主要支持中国境内，国外目的地可能无法规划详细路线，系统会自动显示虚线连接作为参考。
