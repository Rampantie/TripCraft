      ...
      "activities": [
        {
          ...
          "place": "地点/名称",
          "cost": 花费(数字),
          "latitude": 该活动地点的纬度(数字),
          "longitude": 该活动地点的经度(数字)
        }
      ]
    }
  ],
...
- 交通方式要符合用户的交通偏好
- 每天安排3-6个活动（根据旅行节奏调整）
- 每个活动包含时间、地点、简短描述、花费、经纬度坐标
- 每个活动的latitude和longitude字段必须提供该活动地点的准确坐标（数字类型）
- 对于交通类活动，使用起点或终点的坐标；对于住宿、景点、餐饮类活动，使用具体地点的坐标
- 时间安排要合理，考虑交通时间
- 活动要符合目的地特色
...
5. 地图定位要求（重要）：
- tripDetails.latitude 与 tripDetails.longitude：仅输出一个代表本次旅行目的地的坐标（city-level），用于地图单点标记，该坐标应接近城市中心或主要景点聚集区
- 每个活动必须包含 latitude 和 longitude 字段：提供该活动具体地点的准确经纬度坐标，用于路线规划和地图标记
